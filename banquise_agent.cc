...
#define BANQUISE_AGENT_VERSION 0x0001
...
static char *controller_url;
static char *enrollment_token_file;
static char *state_file;
static char *configured_server_uid;
static char *trusted_keys_dir;
static std::vector<Banquise_repository> controller_repositories;
static my_bool refresh_command;
static my_bool agent_enabled= 1;
...
static ulong poll_interval= 60;
static std::thread agent_thread;
static std::mutex agent_sleep_lock;
static std::condition_variable agent_sleep_condition;
static bool agent_shutdown= false;
static std::string agent_uid, agent_credential;
                  ...
                  std::string *error)
{
  ...
  curl_easy_setopt(curl, CURLOPT_USERAGENT, "mariadb-banquise-agent/1.0");
...
static bool controller_post(const std::string &path, const std::string &body,
                            const std::string &credential, bool enrollment,
                            std::string *response_body, long *response_code,
                            std::string *error)
{
  if (!controller_url || !https_url(controller_url))
  {
    *error= "banquise_agent_controller_url must be an HTTPS URL";
    return false;
  }
  std::string base(controller_url);
  while (!base.empty() && base.back() == '/') base.pop_back();
  CURL *curl= curl_easy_init();
  if (!curl) { *error= "Cannot initialize libcurl"; return false; }
  Download download(MAX_CATALOG_BYTES);
  std::string url= base + path;
  std::string auth= enrollment ? "X-Banquise-Enrollment: " + credential :
  ...
  curl_easy_cleanup(curl);
  *response_code= code;
  if (rc != CURLE_OK || download.too_large)
  {
    *error= download.too_large ? "Controller response is too large" :
      (curl_error[0] ? curl_error : curl_easy_strerror(rc));
    return false;
  }
  response_body->swap(download.data);
  return true;
}
                          ...
                          std::string *error)
{
  ...
  for (int i= 0;; ++i)
  {
    ...
    {
      // Shared catalogs may contain MySQL plugins/components. This MariaDB
      // agent ignores those entries while retaining the rest of the catalog.
...
// Only local, administrator-provisioned public keys establish trust.
static bool trusted_keys(std::map<std::string, std::string> *keys, std::string *error)
{
  struct stat st;
  if (!trusted_keys_dir || trusted_keys_dir[0] != '/' ||
      lstat(trusted_keys_dir, &st) || !S_ISDIR(st.st_mode) ||
      st.st_uid != 0 || (st.st_mode & (S_IWGRP | S_IWOTH)))
  { *error= "trusted_keys_dir must be an absolute, root-owned directory without group/other write access"; return false; }
  DIR *dir= opendir(trusted_keys_dir);
  if (!dir) { *error= "Cannot open trusted_keys_dir"; return false; }
  std::vector<std::string> paths;
  while (struct dirent *entry= readdir(dir))
  {
    std::string name= entry->d_name;
    if (name.size() > 4 && name.substr(name.size() - 4) == ".pub")
      paths.push_back(std::string(trusted_keys_dir) + "/" + name);
  }
  closedir(dir);
  if (paths.size() > 128) { *error= "At most 128 trusted .pub keys are supported"; return false; }
  ...
  for (const auto &path : paths)
  {
    ...
    { id+= hex[packet[i] >> 4]; id+= hex[packet[i] & 15]; }
    if (!keys->emplace(id, path).second)
    { *error= "Duplicate trusted key ID: " + id; return false; }
  }
  return true;
}

static bool key_ids_json(std::string *json, std::string *error)
{
  std::map<std::string, std::string> keys;
  if (!trusted_keys(&keys, error)) return false;
  *json= "[";
...
static bool refresh_repositories(const std::vector<Banquise_repository> &repos,
                                 std::string *error)
{
  std::vector<Banquise_repository> resolved= repos;
  std::map<std::string, std::string> keys;
  if (!repos.empty() && !trusted_keys(&keys, error)) return false;
  for (auto &repo : resolved)
  {
    auto key= keys.find(repo.key);
    if (key == keys.end())
    { *error= "Untrusted catalog key ID for repository '" + repo.name + "'"; return false; }
    repo.key= key->second;
  }
  ...
  if (!repo_load_entries(resolved, &combined, load, error, &failures)) return false;
  std::lock_guard<std::mutex> guard(repo_lock);
...
static bool refresh(std::string *error)
{
  ...
  { std::lock_guard<std::mutex> guard(repo_lock); repos= controller_repositories; }
  return refresh_repositories(repos, error);
}
                         ...
                         std::string *error)
{
  ...
  std::string temp_path= final_path + ".banquise-agent.tmp";
                   ...
                   std::string *notice, const std::string &catalog= "")
{
  ...
  std::string backup_path= final_path + ".banquise-agent.backup";
  struct stat st;
...
static bool load_plugin(const std::string &name, std::string *error,
                        std::string *notice, const std::string &catalog= "")
{
  std::lock_guard<std::mutex> operation_guard(operation_lock);
  Repo_entry selected;
  ...
  std::string path= std::string(opt_plugin_dir) + FN_LIBCHAR + selected.soname;
  struct stat st;
  if (lstat(path.c_str(), &st) || !S_ISREG(st.st_mode))
  {
    *error= "Plugin file is not installed: " + selected.soname;
    return false;
  }
  std::string version;
  if (loaded_version(selected.soname, &version))
  {
    *notice= selected.name + " is already loaded";
    return true;
  }
  if (!sql_soname("INSTALL", selected.soname, error)) return false;
  *notice= "Loaded " + selected.name;
  return true;
}

static bool read_agent_state()
{
  if (!state_file || !state_file[0]) return false;
  int fd= open(state_file, O_RDONLY | O_CLOEXEC | O_NOFOLLOW);
  if (fd < 0) return false;
  struct stat st;
  char data[1024];
  ssize_t n= fstat(fd, &st) ? -1 : read(fd, data, sizeof(data));
  close(fd);
  if (n <= 0 || n == (ssize_t) sizeof(data) || !S_ISREG(st.st_mode) ||
      (st.st_mode & (S_IRWXG | S_IRWXO))) return false;
  std::string contents(data, (size_t) n);
  size_t newline= contents.find('\n');
  size_t end= newline == std::string::npos ? newline : contents.find('\n', newline + 1);
  if (newline == std::string::npos) return false;
  agent_uid= contents.substr(0, newline);
  agent_credential= contents.substr(newline + 1, end - newline - 1);
  return safe_name(agent_uid) && agent_credential.size() >= 32;
}

static bool write_agent_state(std::string *error)
{
  if (!state_file || !state_file[0]) { *error= "Agent state file is not configured"; return false; }
  std::string temporary= std::string(state_file) + ".tmp";
  int fd= open(temporary.c_str(), O_WRONLY | O_CREAT | O_EXCL | O_CLOEXEC, 0600);
  if (fd < 0) { *error= "Cannot create agent state: " + std::string(strerror(errno)); return false; }
  std::string contents= agent_uid + "\n" + agent_credential + "\n";
  ssize_t n= write(fd, contents.data(), contents.size());
  bool ok= n == (ssize_t) contents.size() && !fsync(fd) && !close(fd) &&
           !rename(temporary.c_str(), state_file);
  if (!ok)
  {
    int saved= errno; close(fd); unlink(temporary.c_str());
    *error= "Cannot publish agent state: " + std::string(strerror(saved));
  }
  return ok;
}

static bool read_enrollment_token(std::string *token, std::string *error)
{
  if (!enrollment_token_file || !enrollment_token_file[0])
  { *error= "Enrollment token file is not configured"; return false; }
  int fd= open(enrollment_token_file, O_RDONLY | O_CLOEXEC | O_NOFOLLOW);
  if (fd < 0) { *error= "Cannot open enrollment token file"; return false; }
  struct stat st;
  char data[512];
  ssize_t n= fstat(fd, &st) ? -1 : read(fd, data, sizeof(data));
  close(fd);
  if (n <= 0 || n == (ssize_t) sizeof(data) || !S_ISREG(st.st_mode) ||
      st.st_uid != 0 || (st.st_mode & S_IWGRP) || (st.st_mode & S_IRWXO))
  { *error= "Enrollment token must be root-owned, not group-writable, and inaccessible to others"; return false; }
  while (n > 0 && (data[n - 1] == '\n' || data[n - 1] == '\r')) --n;
  token->assign(data, (size_t) n);
  return !token->empty();
}
...
{
  struct utsname info;
  return uname(&info) ? "unknown" : std::string(info.sysname) + " " + info.release;
}

static std::string plugin_report()
{
  std::vector<Repo_entry> snapshot;
  { std::lock_guard<std::mutex> guard(repo_lock); snapshot= entries; }
  std::string result= "[";
  bool first= true;
  for (const Repo_entry &entry : snapshot)
  {
    std::string path= std::string(opt_plugin_dir) + FN_LIBCHAR + entry.soname;
    struct stat st;
    /* Match the INFORMATION_SCHEMA view and MariaDB's loader semantics: a
       symlink that resolves to a regular plugin file is installed. */
    bool installed= !stat(path.c_str(), &st) && S_ISREG(st.st_mode);
    std::string version;
    bool managed= installed && read_installed_version(entry.soname, &version);
    std::string runtime_version;
    bool loaded= loaded_version(entry.soname, &runtime_version);
    if (version.empty()) version= runtime_version;
    if (!first) result+= ',';
    first= false;
    result+= "{\"name\":\"" + json_escape(entry.name) + "\",\"catalog_name\":\"" + json_escape(entry.catalog) + "\",\"installed\":" +
      (installed ? "true" : "false") + ",\"loaded\":" + (loaded ? "true" : "false") +
      ",\"managed\":" + (managed ? "true" : "false") +
      ",\"installed_version\":\"" + json_escape(version) + "\"}";
  }
  return result + "]";
}

static bool ensure_agent_identity(std::string *error)
{
  if (!agent_uid.empty() && !agent_credential.empty()) return true;
  if (read_agent_state())
  {
    if (configured_server_uid && configured_server_uid[0] && agent_uid != configured_server_uid)
    { *error= "Configured server_uid differs from persisted agent identity"; return false; }
    return true;
  }
  if (configured_server_uid && configured_server_uid[0]) agent_uid= configured_server_uid;
  else
  {
    ...
    if (my_random_bytes(random, sizeof(random)) != 0) { *error= "Cannot generate server UID"; return false; }
    static const char hex[]= "0123456789abcdef";
    agent_uid= "banquise-";
    for (unsigned char c : random) { agent_uid+= hex[c >> 4]; agent_uid+= hex[c & 15]; }
  }
  if (!safe_name(agent_uid)) { *error= "Unsafe server UID"; return false; }
  std::string enrollment, response, key_ids;
  if (!key_ids_json(&key_ids, error)) return false;
  if (!read_enrollment_token(&enrollment, error)) return false;
  std::string body= "{\"server_uid\":\"" + json_escape(agent_uid) +
    "\",\"mariadb_version\":\"" + json_escape(server_version) +
    "\",\"os\":\"" + json_escape(operating_system()) +
    "\",\"architecture\":\"" + local_architecture() +
    "\",\"catalog_key_ids\":" + key_ids + "}";
  long code= 0;
  if (!controller_post("/api/v1/agents/register", body, enrollment, true,
                       &response, &code, error)) return false;
  if (code != 201) { *error= "Registration rejected with HTTP " + std::to_string(code) + ": " + response; return false; }
  if (!json_string_field(response.data(), (int) response.size(), "agent_token",
                         &agent_credential, true, error)) return false;
  return write_agent_state(error);
}

static bool acknowledge_task(long long id, bool success, const std::string &result,
                             std::string *error)
{
  std::string body= std::string("{\"success\":") + (success ? "true" : "false") +
    ",\"result\":\"" + json_escape(result) + "\",\"plugins\":" +
    plugin_report() + "}";
  std::string response;
  long code= 0;
  std::string path= "/api/v1/agents/" + agent_uid + "/tasks/" +
                    std::to_string(id) + "/ack";
  if (!controller_post(path, body, agent_credential, false, &response, &code, error)) return false;
  if (code != 200) { *error= "Task acknowledgement returned HTTP " + std::to_string(code); return false; }
  return true;
}

static bool process_poll_response(const std::string &response, std::string *error)
{
  const char *value= NULL;
  int length= 0;
  const char *root= NULL;
  int root_length= 0;
  if (json_type(response.data(), response.data() + response.size(),
                &root, &root_length) != JSV_OBJECT)
  { *error= "Controller response must be a JSON object"; return false; }
  std::string status;
  if (!json_string_field(response.data(), (int) response.size(), "status",
                         &status, true, error)) return false;
  std::vector<Banquise_repository> repos;
  const char *catalogs= NULL;
  int catalogs_length= 0;
  if (json_get_object_key(response.data(), response.data() + response.size(),
                          "catalogs", &catalogs, &catalogs_length) != JSV_ARRAY)
  { *error= "Controller catalogs must be an array"; return false; }
  std::map<std::string, bool> names;
  for (int i= 0;; ++i)
  {
    const char *object= NULL;
    int length= 0;
    auto type= json_get_array_item(catalogs, catalogs + catalogs_length, i, &object, &length);
    if (type == JSV_NOTHING) break;
    Banquise_repository repo;
    if (i >= 128 || type != JSV_OBJECT ||
        !json_string_field(object, length, "name", &repo.name, true, error) ||
        !json_string_field(object, length, "url", &repo.url, true, error) ||
        !json_string_field(object, length, "key_id", &repo.key, true, error))
    { *error= "Invalid controller catalog descriptor"; return false; }
    if (!repo_name_valid(repo.name) || !https_url(repo.url) || repo.key.size() != 16 ||
        repo.key.find_first_not_of("0123456789abcdef") != std::string::npos ||
        !names.emplace(repo.name, true).second)
    { *error= "Invalid or duplicate controller repository"; return false; }
    repos.push_back(repo);
  }
  if (!refresh_repositories(repos, error)) return false;
  { std::lock_guard<std::mutex> guard(repo_lock); controller_repositories= repos; }
  if (status != "active")
  {
    set_message("Agent is " + status + "; catalog prepared, no tasks accepted");
    return true;
  }
  enum json_types type= json_get_object_key(response.data(), response.data() + response.size(),
                                             "tasks", &value, &length);
  if (type != JSV_ARRAY) { *error= "Controller tasks must be an array"; return false; }
  for (int task_index= 0;; ++task_index)
  {
    const char *object= NULL;
    int object_length= 0;
    type= json_get_array_item(value, value + length, task_index,
                              &object, &object_length);
    if (type == JSV_NOTHING) break;
    if (type != JSV_OBJECT)
    { *error= "Every controller task must be an object"; return false; }
    std::string action, plugin, task_catalog;
    const char *id_value= NULL;
    int id_length= 0;
    if (!json_string_field(object, object_length, "action", &action, true, error) ||
        !json_string_field(object, object_length, "plugin_name", &plugin, true, error) ||
        !json_string_field(object, object_length, "catalog_name", &task_catalog, true, error) ||
        json_get_object_key(object, object + object_length, "id", &id_value, &id_length) != JSV_NUMBER)
      return false;
    long long id= strtoll(std::string(id_value, (size_t) id_length).c_str(), NULL, 10);
    std::string operation_error, notice;
    bool success= false;
    if (!names.count(task_catalog)) operation_error= "Task repository is not assigned to this agent";
    else if (action == "install") success= install(plugin, &operation_error, &notice, task_catalog);
    else if (action == "update") success= update(plugin, &operation_error, &notice, task_catalog);
    else if (action == "uninstall") success= uninstall(plugin, &operation_error, &notice, task_catalog);
    else if (action == "load") success= load_plugin(plugin, &operation_error, &notice, task_catalog);
    else operation_error= "Unsupported controller action";
    std::string result= success ? notice : operation_error;
    std::string ack_error;
    if (!acknowledge_task(id, success, result, &ack_error))
      set_message("Task completed but acknowledgement failed: " + ack_error);
  }
  return true;
}

static void agent_cycle()
{
  std::string error;
  if (!ensure_agent_identity(&error)) { set_message("Registration failed: " + error); return; }
  std::string key_ids;
  if (!key_ids_json(&key_ids, &error)) { set_message("Trust configuration failed: " + error); return; }
  std::string body= "{\"mariadb_version\":\"" + json_escape(server_version) +
    "\",\"os\":\"" + json_escape(operating_system()) +
    "\",\"architecture\":\"" + local_architecture() +
    "\",\"catalog_key_ids\":" + key_ids + ",\"last_error\":\"\",\"plugins\":" + plugin_report() + "}";
  std::string response;
  long code= 0;
  std::string path= "/api/v1/agents/" + agent_uid + "/poll";
  if (!controller_post(path, body, agent_credential, false, &response, &code, &error))
  { set_message("Controller poll failed: " + error); return; }
  if (code != 200) { set_message("Controller poll returned HTTP " + std::to_string(code) + ": " + response); return; }
  if (!process_poll_response(response, &error)) set_message("Controller response failed: " + error);
  else if (status_message[0] == 0 || !strstr(status_message, "quarantine"))
    set_message("Controller poll completed successfully");
}

static void agent_main()
{
  if (my_thread_init()) { set_message("Cannot initialize agent thread"); return; }
  std::unique_lock<std::mutex> lock(agent_sleep_lock);
  while (!agent_shutdown)
  {
    ...
    agent_sleep_condition.wait_for(lock, std::chrono::seconds(agent_enabled ? poll_interval : 1),
...
static MYSQL_SYSVAR_STR(trusted_keys_dir, trusted_keys_dir,
  PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY | PLUGIN_VAR_MEMALLOC,
  "Directory of administrator-provisioned trusted Minisign .pub keys", NULL, NULL,
  "/etc/mariadb/banquise/trusted.d");
...
static MYSQL_SYSVAR_STR(state_file, state_file,
  ...
  "/var/lib/mysql/banquise-agent.state");
static MYSQL_SYSVAR_STR(server_uid, configured_server_uid,
  PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY | PLUGIN_VAR_MEMALLOC,
  "Stable server identifier; generated and persisted when empty", NULL, NULL, "");
static MYSQL_SYSVAR_BOOL(enabled, agent_enabled, PLUGIN_VAR_RQCMDARG,
  "Enable registration, heartbeat, and task processing", NULL, NULL, 1);
static MYSQL_SYSVAR_ULONG(poll_interval, poll_interval, PLUGIN_VAR_RQCMDARG,
  "Controller polling interval in seconds", NULL, NULL, 60, 5, 86400, 1);
...
static st_mysql_sys_var *repo_vars[]= {
  MYSQL_SYSVAR(trusted_keys_dir), MYSQL_SYSVAR(controller_url),
  MYSQL_SYSVAR(enrollment_token_file), MYSQL_SYSVAR(state_file), MYSQL_SYSVAR(server_uid),
  MYSQL_SYSVAR(enabled), MYSQL_SYSVAR(poll_interval), MYSQL_SYSVAR(connect_timeout),
  MYSQL_SYSVAR(transfer_timeout), MYSQL_SYSVAR(refresh), NULL
};

static st_mysql_show_var repo_status[]= {
  {"banquise_agent_message", status_message, SHOW_CHAR},
...
static int agent_init(void *)
{
  ...
  catch (...) { curl_global_cleanup(); set_message("Cannot start agent thread"); return 1; }
  set_message(agent_enabled ? "Agent thread started" : "Agent is disabled");
  return 0;
}
...
static int agent_deinit(void *)
{
  ...
  agent_sleep_condition.notify_all();
...
class Item_func_banquise_agent_action : public Item_str_func
{
  Repo_action action;
  bool executed= false;
  ...
  std::string result;
public:
  Item_func_banquise_agent_action(THD *thd, List<Item> &list, Repo_action action_arg):
    Item_str_func(thd, list), action(action_arg) {}

  LEX_CSTRING func_name_cstring() const override
  {
    static LEX_CSTRING names[]= {
      { STRING_WITH_LEN("banquise_agent_install") },
      { STRING_WITH_LEN("banquise_agent_uninstall") },
      { STRING_WITH_LEN("banquise_agent_update") }
    };
    return names[action];
  }
  ...
  String *val_str(String *to) override
  {
    ...
    if (!executed)
    {
      ...
      if (!succeeded)
      {
        ...
        my_printf_error(ER_UNKNOWN_ERROR, "BANQUISE_AGENT: %s", MYF(0),
                        result.c_str());
        null_value= true;
        return NULL;
      }
  ...
  { return get_item_copy<Item_func_banquise_agent_action>(thd, this); }
};

template <Repo_action action> class Create_func_banquise_agent_action :
...
public:
  Item *create_native(THD *thd, const LEX_CSTRING *name, List<Item> *list) override
  {
    ...
    return new (thd->mem_root) Item_func_banquise_agent_action(thd, *list, action);
  }
  static Create_func_banquise_agent_action singleton;
};

template <Repo_action action>
Create_func_banquise_agent_action<action>
  Create_func_banquise_agent_action<action>::singleton;
static Plugin_function install_function_descriptor(
  &Create_func_banquise_agent_action<ACTION_INSTALL>::singleton);
static Plugin_function uninstall_function_descriptor(
  &Create_func_banquise_agent_action<ACTION_UNINSTALL>::singleton);
